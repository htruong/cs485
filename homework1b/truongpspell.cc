...
void * spell_check(void * arg)
{
  ...
  while (true) {
    ...
    // If we don't do that now it will be leaked forever.
    // Can't figure out why gcc doesn't want me to delete word now.
    // delete word;
...
int main (int argc, char * argv[])
{
  ...
  // clever and wait for the input from the dictionary at the same time.
  ...
  if (pthread_create (reader_thread, NULL, read_file_to_vector, rf_param_ptr)) {
    cerr << "Error creating dict reader thread." << endl;
    abort();
  }
  ...
  if (pthread_create (input_thread, NULL, read_input_to_vector, ri_param_ptr)) {
    cerr << "Error creating input reader thread." << endl;
    abort();
  }

  if (pthread_join (*reader_thread, NULL) ) {
    cerr << "Error joining dict reader thread." << endl;
    abort();
  }
  delete reader_thread;

  // As soon as we're done with loading the dict, we can begin spell checking
  ...
  for (uint i = 0; i < nthreads; i++) {
    if (pthread_create (speller_thread[i], NULL, spell_check, sp_param_ptr) ) {
  ...
  if (pthread_join (*input_thread, NULL) ) {
    cerr << "Error joining input reader thread." << endl;
  abort();
